...
def get_uptime_and_downtime(
    ...
    # List to store the overlapping intervals between store hours and store status intervals
    overlaps = []

    i = j = 0

    # Iterate over both lists to find overlapping intervals
    while i < len(store_hours_list) and j < len(store_status_intervals):
        ...
        # Find the maximum start time of the two intervals
        max_start = max(start1, start2)
        # Find the minimum end time of the two intervals
        min_end = min(end1, end2)

        # If there is an overlap between the two intervals, add it to the `overlaps` list
        if max_start < min_end:
            overlaps.append((max_start, min_end, store_status_intervals[j][2]))
        # If the end of the store hours interval is before the end of the store status interval,
        # move to the next store hours interval
        if end1 < end2:
            i += 1
        else:
            # Otherwise, move to the next store status interval
            j += 1

    uptime = downtime = 0
    # Iterate over the overlapping intervals and compute uptime and downtime
...
# TO-DO - Modularize the code for calculating store hours intervals
...
def process_store_data(store_id: str, db: Session) -> Tuple[str, int, int, int, int, int, int]:
    ...
    # Create datetime objects for the current time, 1 hour ago, 1 day ago and 1 week ago
    current_time = datetime(2023, 1, 25, 18, 0, 0, tzinfo=UTC_TIMEZONE)
    ...
    # Example - current day is wednesday 25
    # week_datetimes[2] will be datetime object with date 25
    week_datetimes = {}

    # Loop through 7 days (last week)
    for i in range(7):
        # Calculate the day by subtracting the number of days from the current day
        # using the modulo operator to ensure it stays within a range of 0-6 (days of the week)
        ...
        # using the weekday as the key, so that the week_datetimes[2] will return the datetime object for Wednesday
    ...
    if not store_hours:
        for day in range(7):
            # Calculate the start time of each day as midnight in UTC timezone
            start_time = (
                week_datetimes[day].replace(hour=0, minute=0, second=0, microsecond=0).astimezone(UTC_TIMEZONE)
            )
            # Calculate the end time of each day as midnight of the next day in UTC timezone
            end_time = (
    ...
    # If store hours are found, loop through each store hours interval
    else:
